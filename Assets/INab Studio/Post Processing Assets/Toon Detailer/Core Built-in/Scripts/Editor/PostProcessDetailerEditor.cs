...
{
    [CustomEditor(typeof(PostProcessDetailer))]
    public class PostProcessDetailerEditor : Editor
    {
        ...
        private SerializedProperty _FastMode;
        ...
        private void OnEnable()
        {
            ...
            _FastMode = settings.FindPropertyRelative("_FastMode");
        ...
        private void DrawGeneral()
        {
            using (new EditorGUILayout.VerticalScope(EditorStyles.helpBox))
            {
                ...
                if (_StencilUse.enumValueIndex != 0)
                {
                    EditorGUILayout.PropertyField(_StencilMaskLayer);
                }
                EditorGUILayout.PropertyField(_FastMode);
                if (GUILayout.Button("Update"))
                {
                    var t = (target as PostProcessDetailer);
                    t.enabled = false;
                    t.enabled = true;
                }
            }
